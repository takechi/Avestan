...
namespace
{
	static string FunctionError(PCWSTR fn)
	{
		...
		return string::format(L"ScriptError: $1 の呼び出しでエラーが発生しました。（$2: $3）",
			fn, pygmy::String(error.exception).str(), pygmy::String(error.value).str());
	}
	static string FunctionNotFound(PCWSTR fn)
	{
		return string::format(L"ScriptError: $1 が見つかりません。", fn);
	}
...
public: // ICallback
	string Caption(const string& name, const string& path)
	{
		...
		{
			PyErr_Clear();
			if(pygmy::String result = m_fnCaption(StringOrNone(name), StringOrNone(path)))
				return result.str();
	...
	Navigation NavigateVerb(IEntry* current, IEntry* where, UINT mods, bool locked, Navigation defaultVerb)
	{
		if(m_fnNavigateVerb)
		{
			...
			PyErr_Clear();
			pygmy::Int result = m_fnNavigateVerb(
				PathOrNone(current),
				PathOrNone(where),
				pygmy::Int(mods),
				pygmy::Bool(locked),
	...
	string ExecuteVerb(IEntry* current, IEntry* what, UINT mods)
	{
		...
		{
			PyErr_Clear();
			if(pygmy::String result = m_fnExecuteVerb(
				PathOrNone(current),
				PathOrNone(what),
				pygmy::Int(mods)))
			{
...
namespace
{
	...
	static PyMethodDef pygmy_functions[] =
	{
