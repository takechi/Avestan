...
void mew::io::StreamReadExact(IStream* stream, void* buffer, size_t size)
{
	...
	if(done != size)
		throw IOError(_T(__FUNCTION__), STG_E_READFAULT);
...
size_t mew::io::StreamReadSome(IStream* stream, void* buffer, size_t size)
{
	...
	if FAILED(hr)
		throw IOError(_T(__FUNCTION__), hr);
	return done;
}
...
void mew::io::StreamWriteExact(IStream* stream, const void* buffer, size_t size)
{
	...
	if(done != size)
		throw IOError(_T(__FUNCTION__), STG_E_MEDIUMFULL);
...
void mew::io::StreamSeekRel(IStream* stream, INT64 mov, UINT64* newpos)
{
	StreamSeek(stream, STREAM_SEEK_CUR, mov, newpos);
}
...
#define CSIDL_PLURA( name )			{ L#name, CSIDL_##name##S }, { L#name L"S", CSIDL_##name##S }
...
#undef CSIDL_ENTRY
#undef CSIDL_ALIAS
	static bool operator < (const CSIDL_Map& lhs, PCWSTR rhs)
	{
		return _wcsicmp(lhs.name, rhs) < 0;
	}
	static bool operator == (const CSIDL_Map& lhs, PCWSTR rhs)
	{
		return _wcsicmp(lhs.name, rhs) == 0;
	...
	static void CreateFileStream(REFINTF pp, IUnknown* arg, DWORD stgm, UINT nErrorID) throw(...)
	{
		if(string path = PathFromArg(arg))
		{
			...
			if FAILED(hr = SHCreateStreamOnFile(path.str(), stgm, &stream))
				throw IOError(string::load(IDS_ERR_OPENFILE, path), hr);
			if FAILED(hr = stream.copyto(pp))
				throw CastError(string::load(IDS_ERR_NOINTERFACE, stream, pp.iid), hr);
		}
		else
		{
