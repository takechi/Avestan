...
public:
	...
	void __init__(IUnknown* arg)
	{
		if(string filename = cast(arg))
		{
			...
			if(!m_normal)
				throw IOError(string::load(IDS_ERR_IMAGELIST, filename), STG_E_FILENOTFOUND);
		...
		else if(arg)
		{
			...
			if(!m_normal)
				throw IOError(string::load(IDS_ERR_IMAGELIST, L"[stream]"), STG_E_FILENOTFOUND);
			// m_disabled = null;
			// m_hot = null;
		}
		else
...
public: // IImageList
	...
	HRESULT __stdcall AddMasked(HBITMAP hbmImage, COLORREF crMask, int *pi)	{ return call(pi, ImageList_AddMasked(m_normal, hbmImage, crMask)); }
...
//		return call(ImageList_Merge(m_normal, i1, punk2, i2, dx, dy, iid, ppv));
	...
	{
		return FromHIMAGELIST(
			m_normal.Duplicate(),
			m_disabled.Duplicate(),
			m_hot.Duplicate(),
			iid, ppv);
	}
	...
	HRESULT __stdcall BeginDrag(int iTrack, int dxHotspot, int dyHotspot)							{ return call(ImageList_BeginDrag(m_normal, iTrack, dxHotspot, dyHotspot)); }
	HRESULT __stdcall EndDrag(void)																	{ ImageList_EndDrag(); return S_OK; }
	...
	HRESULT __stdcall SetDragCursorImage(IUnknown *punk, int iDrag, int dxHotspot, int dyHotspot)	{ return call(ImageList_SetDragCursorImage(m_normal, iDrag, dxHotspot, dyHotspot)); }
	HRESULT __stdcall DragShowNolock(BOOL fShow)													{ return call(ImageList_DragShowNolock(fShow)); }
	HRESULT __stdcall GetDragImage(POINT *ppt, POINT *pptHotspot, REFIID iid, PVOID *ppv)			{ return FromHIMAGELIST(ImageList_GetDragImage(ppt, pptHotspot), null, null, iid, ppv); }
