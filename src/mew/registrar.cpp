...
inline bool operator < (REFGUID lhs, REFGUID rhs)
{
...
namespace mew
{
	MEW_API void CreateInstance(REFCLSID clsid, REFINTF ppInterface, IUnknown* arg) throw(...)
	{
		...
		if(i != GetRegistrar().end())
		{	// ファクトリ関数が見つかった
			i->second(ppInterface, arg);
			ASSERT(*ppInterface.pp);
			return;
		}
		else if(SUCCEEDED(hr = ::CoCreateInstance(clsid, null, CLSCTX_ALL, ppInterface.iid, ppInterface.pp)))
		{	// COMクラスの作成に成功
