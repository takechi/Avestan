...
{
private:
	typedef std::allocator< std::pair<GUID, variant> >		VariantAlloc;
	typedef std::map<Guid, variant, std::less<Guid>, VariantAlloc>	VariantMap;
...
private:
	...
	public:
		...
		bool Next(GUID* key, variant* var)
		{
			...
			if(var)	{ *var = m_iter->second; }
			++m_iter;
			return true;
		}
		void Reset()
		{
			m_iter = m_owner->m_vars.begin();
		}
	};
