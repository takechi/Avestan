...
#ifdef _DEBUG
#define TRACE_FORMAT(fmt, cf)	if(fmt->cfFormat == cf)	TRACE(L#cf); else
...
namespace
{
	...
	static HRESULT DuplicateStgMedium(const STGMEDIUM* src, STGMEDIUM* dst, const FORMATETC* fmt, DWORD tymed = 0)
	{
		...
		else
		{
			...
			case TYMED_HGLOBAL:
				...
				case TYMED_ISTREAM: // hGlobal to IStream
					if FAILED(hr = CreateStreamOnHGlobal(src->hGlobal, false, &dst->pstm))
						return hr;
					break;
				default:
					return OLE_E_CANTCONVERT;
		...
		if((dst->pUnkForRelease = src->pUnkForRelease) != null)
	...
	public: // IDropSource
		STDMETHODIMP QueryContinueDrag(BOOL bEscapePressed, DWORD mk)
		{
			...
			if (bEscapePressed
			|| ((mk & MK_LBUTTON) && (mk & (MK_MBUTTON | MK_RBUTTON)))
			|| ((mk & MK_MBUTTON) && (mk & (MK_LBUTTON | MK_RBUTTON)))
			|| ((mk & MK_RBUTTON) && (mk & (MK_LBUTTON | MK_MBUTTON))))
...
private:
	...
	public:
		...
		bool IsCompatible(const FORMATETC* fmt) const
		{
			...
			{
				return fmt->cfFormat == CF_INETURLA
					|| fmt->cfFormat == CF_INETURLW;
			}
			else
				return false;
		}
		bool CanConvert(const FORMATETC* fmt) const
		{
			...
			{
				return fmt->cfFormat == CF_SHELLIDLIST
		...
		HGLOBAL ConvertCopy(CLIPFORMAT fmt) const
		{
			...
			{
				if(fmt == CF_UNICODETEXT || fmt == CF_INETURLW)
				{	// A => W
...
public: // IDragSource
	...
	{
		return AddData(CF_SHELLIDLIST, pCIDA, afx::CIDAGetSize(pCIDA));
	}
...
public: // IDataObject
	...
	{
		ATLTRACENOTIMPL(_T("DragSource::DUnadvise\n"));
	}
	STDMETHOD(EnumDAdvise)(IEnumSTATDATA **ppenumAdvise)
	{
		ATLTRACENOTIMPL(_T("DragSource::EnumDAdvise\n"));
	}
};
