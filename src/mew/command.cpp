...
namespace mew
{
	...
	template <> struct Event<EventQueryState>
	{
...
namespace
{
	...
	public: // Object
		Command(string name, string description) : m_Name(name), m_Description(description)
		{
	...
	public: // ISignal
		...
		struct Compare
		{
			...
			{
				return operator () (lhs->m_Name, rhs->m_Name);
			}
			bool operator () (const string& lhs, const Command* rhs) const
			{
				return operator () (lhs, rhs->m_Name);
			}
			bool operator () (const Command* lhs, const string& rhs) const
			{
				return operator () (lhs->m_Name, rhs);
			}
		};
	};
...
public: // ICommands
	...
	HRESULT Find(REFINTF ppInterface, string name)
	{
		...
		if(i == m_map.end())
		{
			...
			while(string nm = token.next())
			{
				...
				if(!cmd)
					cmd.attach(new CompoundCommand(name));
	...
	{
		return m_map.erase(name) > 0 ? S_OK : E_FAIL;
	}
