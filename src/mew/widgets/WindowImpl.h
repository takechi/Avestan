...
private:
	CWindowEx	m_window;
public:
	SuppressRedraw(HWND hWnd) : m_window(hWnd)
	{
...
#ifdef _DEBUG
		if(IsBadReadPtr(piid, sizeof(GUID)) || IsBadWritePtr(ppObject, sizeof(void**)))
		{	// 間違って呼ばれる可能性があるので、防御は完璧に…
			__debugbreak();
			return E_POINTER;
		}
#endif
...
public: // overridable
	...
	{
		return m_Extensions.ProcessQueryDrop(pp);
	}

public: // Object
	WindowImpl()
	{
		DEBUG_ONLY( TRACE(_T("$1()"), GetFinalClassName()) );
	}
	~WindowImpl()
	{
		DEBUG_ONLY( TRACE(_T("~$1()"), GetFinalClassName()) );
...
public: // IWindow
	...
	{
		if SUCCEEDED(super::Send(this, msg))
			return S_OK;
...
public:
	BOOL ProcessWindowMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lResult, DWORD dwMsgMapID = 0)
	{
		...
		case 0:
			...
			case WM_CREATE:
				lResult = DefWindowProc();
				if(!final.HandleCreate(*(LPCREATESTRUCT)lParam))
					lResult = -1;
				break;
			default:
				break;
	...
	virtual BEGIN_MSG_MAP_(HandleWindowMessage)
		CHAIN_MSG_MAP_TO(super::ProcessWindowMessage)
		// self
		...
		// msg source
		MESSAGE_HANDLER(MEW_ECHO_COPYDATA	, OnCopyDataEcho)
		MESSAGE_HANDLER(WM_CLOSE			, OnClose)
		MESSAGE_HANDLER(WM_ENDSESSION		, OnEndSession)
		MESSAGE_HANDLER(WM_SETTEXT			, OnSetText)
		MESSAGE_HANDLER(WM_SIZE				, OnSize)
		// query
		MESSAGE_HANDLER(MEW_QUERY_INTERFACE	, OnQueryInterface)
		MESSAGE_HANDLER(MEW_QUERY_GESTURE	, OnQueryGesture)
	...
	{
		MSG msg;
		if(!::PeekMessage(&msg, m_hWnd, MEW_ECHO_UPDATE, MEW_ECHO_UPDATE, PM_NOREMOVE))
		{	// メッセージキューにこれ以上MEW_ECHO_UPDATEが無い場合のみ処理する。
			// 複数回レイアウト更新された場合に、無駄に再配置するのを防ぐ.
			final.HandleUpdateLayout();
		}
		else if(msg.hwnd != m_hWnd)
		{	// 自分の子供ウィンドウのメッセージも見つけてしまうので、特別処理
