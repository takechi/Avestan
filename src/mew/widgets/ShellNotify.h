...
class SHNotifyBase {
 ...
 public:
  ...
  HRESULT SHBeginMonitor(int numEntries, SHChangeNotifyEntry entries[], HWND hWnd, UINT uMsg, LONG shcneMask,
                         int shcnrf = SHCNRF_ShellLevel | SHCNRF_NewDelivery) {
    SHEndMonitor();
  ...
  HRESULT SHEndMonitor() {
    if (m_nSHChangeNotifyID == 0) return S_FALSE;
