...
namespace mew
{
	template <> struct Event<EventFolderChanging>
	{
	...
	template <> struct Event<EventFolderChange>
	{
		static void event(message& msg, IWindow* from, io::IEntry* where)
		{
			msg["from" ] = from;
	...
	template <> struct Event<EventExecuteEntry>
	{
	...
	template <> struct Event<EventStatusText>
	{
...
class ShellListView :
	public WindowImpl
	<
		CWindowImplEx< ShellListView, Shell >,
		implements<IShellListView, IListView, IList, IWindow, ISignal, IDisposable, IGesture, IWallPaper, IDropTarget>
	>,
	public SHNotifyBase
{
public:
	...
	{	// default gesture
		...
		if(length == 1 && gesture[0] == GestureButtonRight)
			return S_OK;
#endif
		return E_FAIL;
	}
	...
	HRESULT OnGestureFinish(UINT16 modifiers, size_t length, const Gesture gesture[])
	{
		...
		switch(gesture[1])
		{
		case GestureEast:	Go(DirEast);	return S_OK;
		case GestureWest:	Go(DirWest);	return S_OK;
		case GestureNorth:	Go(DirNorth);	return S_OK;
		case GestureSouth:	Close();		return S_OK;
		}
#endif
		return E_INVALIDARG;
	}
...
public: // msg map
	...
	HRESULT Send(message msg)
	{
		...
		switch(msg.code)
		{
		case CommandKeyUp:			MimicKeyDown(VK_UP   , 0); break;
		case CommandKeyDown:		MimicKeyDown(VK_DOWN , 0); break;
		case CommandKeyLeft:		MimicKeyDown(VK_LEFT , 0); break;
		case CommandKeyRight:		MimicKeyDown(VK_RIGHT, 0); break;
		case CommandKeyHome:		MimicKeyDown(VK_HOME , 0); break;
		case CommandKeyEnd:			MimicKeyDown(VK_END  , 0); break;
		case CommandKeyPageUp:		MimicKeyDown(VK_PRIOR, 0); break;
		case CommandKeyPageDown:	MimicKeyDown(VK_NEXT , 0); break;
		case CommandKeySpace:		MimicKeyDown(VK_SPACE , 0); break;
		case CommandKeyEnter:		MimicKeyDown(VK_RETURN, 0); break;
		case CommandCut:			Cut();				break;
		case CommandCopy:			Copy();				break;
		case CommandPaste:			PasteEx();			break;
		case CommandDelete:			Delete();			break;
		case CommandBury:			Bury();				break;
		case CommandUndo:			Undo();				break;
		case CommandRename:			Rename();			break;
		case CommandSelectChecked:	SelectChecked();	break;
		case CommandSelectAll:		SelectAll();		break;
		case CommandSelectNone:		SelectNone();		break;
		case CommandSelectReverse:	afx::ListView_SelectReverse(this->ListView);	break;
		case CommandSelectToFirst:	SelectToFirst();	break;
		case CommandSelectToLast:	SelectToLast();		break;
		case CommandCheckAll:		CheckAll();			break;
		case CommandCheckNone:		CheckNone();		break;
		case CommandProperty:		Property();			break;
		case CommandGoBack:			Go(DirWest);		break;
		case CommandGoForward:		Go(DirEast);		break;
		case CommandGoUp:			Go(DirNorth);		break;
		case CommandSave:			Shell::SaveViewState();	break;
	...
	void PasteEx()
	{
		...
		ref<IEntry> item;
		if(theAvesta->PasteInFolder
		&& get_SelectedCount() == 1
		&& SUCCEEDED(GetContents(&entries, SELECTED))
		&& SUCCEEDED(entries->GetAt(&item, 0))
		&& item->IsFolder())
		{
	...
	LRESULT OnContextMenu(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		{	// アーカイブX は、選択項目が無くても選択項目に対するコンテキストメニューを取得できてしまうため
		...
		{	// 選択アイテムがある場合
			...
			// XP 名前の変更(&M) BUG ... じゃないかも。名前の変更は、ビューによって操作が異なるので、デフォルトでは実装されていないのか？
			TCHAR text[MAX_PATH];
		...
		else if(null != (popup = afx::SHBeginContextMenu(m_pShellView, SVGIO_BACKGROUND, &menu)))
		{	// 選択アイテムが無い場合
	...
	void CopyMenu(HMENU to, HMENU hFromParent, int nFronIndex)
	{
		...
		for(int i = 0; i < count; ++i)
		{
			...
			if(::GetMenuItemInfo(from, i, true, &getsize))
			{
                ...
                MENUITEMINFO info = { sizeof(MENUITEMINFO), MIIM_BITMAP | MIIM_CHECKMARKS | MIIM_DATA | MIIM_FTYPE | MIIM_ID | MIIM_STATE | MIIM_STRING | MIIM_SUBMENU };
	...
	UINT OnContextMenu_Selection(IContextMenu* menu, CMenuHandle popup, Point ptScreen, bool special)
	{
		...
		ref<IEntryList> entries;
		ref<IEntry> focus;
		if(SUCCEEDED(Shell::GetContents(&entries, FOCUSED))
		&& SUCCEEDED(entries->GetAt(&focus, 0)))
		{
			ref<IEntry> resolved;
	...
	{
		Point ptScreen(GET_XY_LPARAM(lParam));
		if(HitTestComboIcon(ptScreen) && GetCurrentEntry())
		{	// on Icon
			ref<IContextMenu> menu;
			if SUCCEEDED(GetCurrentEntry()->QueryObject(&menu))
			{	// 選択アイテムがある場合
...
public: // overridable
	...
	HRESULT Go(IEntry* folder)
	{
		return Shell::GoAbsolute(folder);
	}
	...
	HRESULT GetStatus(IndexOr<IUnknown> item, DWORD* status, size_t* index = null)
	{
		...
		if(status)
		{
			*status = 0;
			...
			if(lvs & LVIS_FOCUSED)		{ *status |= FOCUSED; }
			if(lvs & LVIS_SELECTED)		{ *status |= SELECTED; }
			if(get_CheckBox() && list.GetCheckState(i))	{ *status |= CHECKED; }
		}
		return S_OK;
	}
	...
	LRESULT OnGetFont(UINT, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		return (LRESULT)m_ListFont.m_hFont;
	}
...
protected: // Shell override
	...
	{
		if(wParam & MK_SHIFT)
		{	// Shiftが押されたままだと、別ウィンドウで開いてしまうので……
		...
		{	// もしShift or Control が押されている場合は、通常のスクロールを行わない
	...
	{
		if(go == GoReplace || !GetCurrentEntry())
		{	// ファイルシステム変更による自動移動 または 初めての移動 の場合はイベントを発生させない
			return true;
		}
		message reply = InvokeEvent<EventFolderChanging>(this, entry);
		bool cancel = reply["cancel"] | false;
		if(cancel)
			TRACE(_T("directory change is canceled"));
		return !cancel;
	}
...
public:
	LRESULT OnDeviceChange(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		case DBT_DEVICEREMOVECOMPLETE: // デバイスが取り出された
			if(IEntry* entry = GetCurrentEntry())
			{	// 何が起こったのかを調べてもいいけど、Exists() を見るのが一番簡単なので、とりあえず。
				if(!entry->Exists())
				{
...
public: // IDropTarget
	STDMETHODIMP DragEnter(IDataObject* src, DWORD key, POINTL pt, DWORD* effect)
	{
		if(ref<IDropTarget> dst = QueryFolderDropTarget())
		{
			...
			if SUCCEEDED(dst->DragEnter(src, key, pt, &dwEffectCopy))
			{	// こっちでエフェクトを設定する
	...
	STDMETHODIMP DragOver(DWORD key, POINTL pt, DWORD* effect)
	{
		...
		{
			DWORD dwEffectCopy = *effect; // DragOver()を呼ぶとゼロにされてしまうので、
			if SUCCEEDED(dst->DragOver(key, pt, &dwEffectCopy))
			{	// こっちでエフェクトを設定する
