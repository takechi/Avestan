...
namespace
{
	...
	struct MenuInfo : MENUINFO
	{
		...
		ITreeItem* get_Root() const
		{
			return (ITreeItem*)dwMenuData;
		}
		void set_Root(ITreeItem* value)
		{
			dwMenuData = (ULONG_PTR)value;
		}
	...
	static void Menu_Insert(HMENU hMenu, int index, ITreeItem* pMenu)
	{	// オーナードローするので、dwTypeData（項目名）は、基本的には表示では使わない。
		...
		// コマンド付き非終端メニュー
		if(pMenu->Command)
		{	// オーナードローにし、WM_MEASUREITEM でゼロを返すことで、
	...
	static void Menu_InvokeCommand(HMENU hMenu, UINT wID)
	{
		...
		{
			if(ICommand* command = pMenu->Command)
				command->Invoke();
		}
	}
	...
	HRESULT Dispatch_WM_MENUCOMMAND(HWND hWnd, ICommand** ppCommand)
	{
		...
		while(::PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
		{
			if(msg.hwnd == hWnd && msg.message == WM_MENUCOMMAND)
			{
				...
				if(ITreeItem* pMenu = Menu_FromHandle((HMENU)msg.lParam, index, true))
...
void MenuProvider::DisposeMenu(HMENU hMenu, bool destroy)
{
	Menu_Dispose(hMenu, destroy);
}
