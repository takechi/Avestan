...
namespace
{
	...
	inline bool IsVisible(const TBBUTTONINFO& info)
	{
		return (info.fsState & TBSTATE_CHECKED) != 0;
	}
	inline bool IsLocked(const TBBUTTONINFO& info)
	{
...
namespace
{
	...
	public:
		...
		{
			theme::TabDrawItem(dc, bounds, text, status, m_FontNormal, GetFocusFont(), m_ColorActiveTab, m_ColorActiveText, m_ColorInactiveText);
		}
		...
		void OnSettingChange(HFONT hFont)
		{
			...
			if(MaxColorDistance(m_ColorBkgnd, m_ColorInactiveText) < 16)
			{	// 背景と非アクティブ色が近すぎる
...
class TabPanel :
	public WindowImpl
	<
		CWindowImplEx< TabPanel, CTypedToolBar<HWND, CToolBarCtrlT<WindowImplBase> > >,
...
public:
	...
	{
		return QueryInterfaceInWindow(GetItemData(index+1), pp);
	}
	InsertTo get_InsertPosition()
	{
		return m_InsertPosition;
	}
	void set_InsertPosition(InsertTo pos)
	{
		m_InsertPosition = pos;
	}
	ArrangeType get_Arrange()
	{
		return m_Arrange;
	}
	...
	HRESULT DoSetStatus(size_t index, Status status = SELECTED, bool unique = false)
	{
		...
		{
			if(status == ToggleSelect)
				status = (GetChecked(index) ? UNSELECTED : SELECTED);
			switch(status)
			{
			case FOCUSED:
				FocusChild(index);
			...
			case SELECTED:
				...
				break;
			case UNSELECTED:
				if(GetCheckedCount() <= 1 || !GetEnabled(index) || !GetChecked(index))
					return E_ABORT;
...
//		SetExtendedStyle(TBSTYLE_EX_DRAWDDARROWS);
	...
	LRESULT OnParentNotify(UINT, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		case WM_DESTROY:
			if(::GetParent((HWND)lParam) == m_hWnd)
				HandleChildCreate(uMsg == WM_CREATE, (HWND)lParam);
			break;
		}
		bHandled = false;
		return 0;
	...
	void HotPopup_OnTimer()
	{
		...
		//
		m_wndHot.clear();
		if(hotIndex.size() < 1 || (shown == 1 && hotIndex.size() == 1 && GetChecked(hotIndex[0])))
		{	// ただ一つが表示されている場合は、通知しなくても明確。
			return;
		}
	...
	{
		if(!m_wndFocus.IsWindow() || !GetChecked(IndexFromParam(m_wndFocus)))
		{	// 以前フォーカスを持っていたビューがすでに無効になっている
			SetFocusWindow(FindFocusTarget());
		}
		if(m_wndFocus)
			m_wndFocus.SetFocus();
	...
	LRESULT OnMouseMove(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if(index > 0 && m_DraggingTab != index)
		{
			...
			|| (m_DraggingTab > index && (rc0.right + rc1.left) / 2 > pt.x))
			{	// チャタリングを防ぐための判定
	...
	{	// 右ボタンが押されていると不思議な動作をするため、いつも押していないとみなす
		SetFocus();
		m_DraggingTab = -1;
		m_DragSwap = false;
		m_RButtonDown = false;
		if(!(GET_KEYSTATE_WPARAM(wParam) & ~MK_LBUTTON) && GetItemCount() > 1)
		{	// 左ボタンのほかには何も押されていない
			Point pt(GET_XY_LPARAM(lParam));
			int index = HitTest(&pt);
			if(index > 0)
...
//			SetCapture();
	...
	LRESULT OnRButtonUp(UINT, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		{
			if(m_RButtonDown)
			{	// 右が押され、その後左が押されることなく右が離された場合のみコンテキストメニュー
	...
	void PopupContextMenu(int index, POINT ptScreen, UINT flags = 0)
	{
		...
		menu.AppendMenu(MF_STRING | (m_Arrange == ArrangeAuto ? MF_CHECKED | MF_GRAYED : 0), ID_ARRANGE_AUTO, L"自動的に並べる(&A)");
		...
		case ID_CLOSE_SHOWN:
			for(int i = 1; i < count; ++i)
				CloseViewCheckVisible(i, true);
			break;
		case ID_CLOSE_HIDDEN:
			for(int i = 1; i < count; ++i)
				CloseViewCheckVisible(i, false);
			break;
	...
	LRESULT OnContextMenu(UINT, WPARAM, LPARAM lParam, BOOL&)
	{
		...
		POINT ptClient = ptScreen; ScreenToClient(&ptClient);
		int index = HitTest(&ptClient);
	...
	HWND _getViewIfClosable(int index) const
	{
		...
		if(!GetButtonInfo(index, &info))
			return null;
		if(!avesta::GetOption(BoolLockClose) && IsLocked(info))
			return null;
...
private:
	...
	string UpdateTabsText(int index, string name)
	{
		if(theAvesta->DistinguishTab)
		{
			...
			for(int i = 0; i < count; ++i)
			{
				...
				while( str::equals_nocase(
					GetTextForTab(newTab.text, minLevel),
					GetTextForTab(otherTab.text, minLevel)) )
				{ ++minLevel; }
	...
	static PCTSTR GetTextForTab(string name, int level)
	{
		...
		int ln = name.length();
		if((ln >= 2 && (text[1] == _T(':') || (text[0] == _T('\\') && text[1] == _T('\\'))))
		|| afx::PathIsRegistory(text))
		{
...
public: // IDropTarget
	...
	STDMETHODIMP DragOver(DWORD key, POINTL pt, DWORD* pdwEffect)
	{
		...
		{	// ターゲットが代わったので、IDropTargetをエミュレートする
			if(m_drop)
				m_drop->DragLeave();
			m_drop = pDropTarget;
			if(m_drop)
