...
namespace
{
	...
	class __declspec(novtable) ToolBarBase :
		public CTypedToolBar< LPARAM, CToolBarCtrlT<WindowImplBase> >,
		public MenuProvider
	{
	protected:
	...
	public:
		...
		{}
		int GetButtonCountWithoutDummy() const
		{
			return GetButtonCount() - BTNOFF;
		}
		...
		void PostKeyDown(WPARAM vkey)
		{
			PostMessage(WM_KEYDOWN, vkey, 0);
		}
	...
	protected:
		...
		{
			if(ref<ICommand> command = IdToCommand(wID))
				command->Invoke();
			GiveFocusBack();
		}
		void DoPopupMenu(const int index, bool animate)
		{
			...
			if(menu->HasChildren())
			{
				...
				ref<ICommand> command = DoTrackPopupMenu(menu, animate ? TPM_VERPOSANIMATION : TPM_NOANIMATION, rcExclude.left, rcExclude.bottom, rcExclude);
				if(info.fsStyle & BTNS_WHOLEDROPDOWN)
...
//			else
		...
		LRESULT OnHookMouseMove(UINT, WPARAM, LPARAM, BOOL& bHandled)
		{
			...
			if(m_bMenuActive && m_bAutoPopup && ::WindowFromPoint(point) == m_hWnd)
			{
				if(CWindowEx wndMenu = ui::GetMenu())
				{
					...
					if((point.x != s_point.x || point.y != s_point.y) && nHit >= 0 && nHit < GetButtonCount() && nHit != m_nPopBtn && m_nPopBtn != -1)
					{
						TBBUTTON tbb;
						GetButton(nHit, &tbb);
	...
	public:
		...
		{
			__super::DoCreate(hParent, NULL, DirNorth, WS_CONTROL | CCS_NOLAYOUT | TBSTYLE_FLAT | TBSTYLE_LIST | TBSTYLE_TRANSPARENT, 0);
...
public: // msg map
	...
	LRESULT OnContextMenu(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		{
			ptClient = ptScreen; ScreenToClient(&ptClient);
	...
	UINT PopupContextMenu(IContextMenu* menu, CMenuHandle popup, POINT ptScreen, int index)
	{
		...
		for(int i = popup.GetMenuItemCount()-1; i >= 0; --i)
		{
			...
			if(popup.GetMenuString(i, text, MAX_PATH, MF_BYPOSITION) == 0)
				continue;
			if(str::equals(text, _T("ショートカットの作成(&S)"))
			|| str::equals(text, _T("削除(&D)"))
...
public: // IDropTarget
	...
	{
		Point ptClient(x, y); ScreenToClient(&ptClient);
		int index = HitTest(&ptClient);
		...
		if(index > 0 && SUCCEEDED(m_root->GetChild(&folder, index-1)))
		{
			...
			{
				ref<IEntry> linked;
				if SUCCEEDED(e->GetLinked(&linked))
					return linked;
			}
		}
		return null;
