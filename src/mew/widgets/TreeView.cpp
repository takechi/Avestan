...
const DWORD MEW_WS_TREE    = WS_CONTROL | WS_VSCROLL | WS_HSCROLL | TVS_HASBUTTONS | TVS_HASLINES | TVS_SHOWSELALWAYS | TVS_EDITLABELS;
...
public:
	...
	{
		InvokeEvent<EventItemFocus>(this, param);
	}
...
public: // ITreeView
	...
	HRESULT GetStatus(IUnknown* item, DWORD* status)
	{
		TRESPASS_DBG( return E_NOTIMPL );
	}
...
public:
	...
	{
//		DoDragDrop(&data, DROPEFFECT_COPY | DROPEFFECT_MOVE | DROPEFFECT_LINK | DROPEFFECT_SCROLL, notify->hdr.hwndFrom, notify->ptDrag);
	}
	...
	HRESULT ExpandAndFocus(const ITEMIDLIST* pidl, HTREEITEM hTreeItem)
	{
		...
		for(HTREEITEM hItem = GetNextItem(hTreeItem, TVGN_CHILD); hItem; hItem = GetNextItem(hItem, TVGN_NEXT))
		{
			if(IFolder* folder = GetItemData(hItem))
			{
				...
				{
					if SUCCEEDED(ExpandAndFocus(pidl, hItem))
						return S_OK;
					EnsureVisible(hItem);
					return E_FAIL;
				}
...
public: // msg map
	...
	HRESULT Send(message msg)
	{
		...
		case CommandPaste:		Paste();		break;
		case CommandDelete:		Delete(true);	break;
		case CommandBury:		Delete(false);	break;
...
public: // file operations
	...
	bool OnBeginLabelEdit(HTREEITEM hItem, ParamType param, PCTSTR text)
	{
		...
		if(param)
		{
			...
			if SUCCEEDED(param->Entry->QueryObject(&psf))
				SHLimitInputEdit(edit, psf);
		}
		return true;
	}
	bool OnEndLabelEdit(HTREEITEM hItem, ParamType param, PCTSTR text)
