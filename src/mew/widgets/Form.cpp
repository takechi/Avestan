...
namespace 
{
	...
	class __declspec(novtable) DockBase :
		public WindowImpl< CWindowImplEx< TFinal, WindowImplBase >, TList >
	{
	protected:
	...
	public:
		...
		{
			bool operator () (IWindow* lhs, IWindow* rhs) const
			{
		...
		void UpdateLayout(Rect rc)
		{
			...
			{
				ref<IWindow> view;
				if(FAILED(QueryInterfaceInWindow(w, &view)) || !view->Visible || view->Dock == DirNone)
					continue;
				views.insert(view);
			}
			if(views.empty())
	...
	public: // msg map
		...
		LRESULT OnSetFocus(UINT, WPARAM wParam, LPARAM, BOOL& bHandled)
		{
			...
			if(m_wndLastFocus != lost && m_wndLastFocus.IsWindow() && m_wndLastFocus.IsWindowVisible() && m_wndLastFocus.IsWindowEnabled())
			{
...
class DockPanel :
	public DockBase< DockPanel, implements<IWindow, ISignal, IDisposable> >,
	public CSplitter<DockPanel>
{
public:
...
public: // message map
	...
	void OnDraw(HDC hDC, const RECT& rcUpdate)
	{
...
class Form :
	public DockBase< Form, implements<IForm, ITree, IWindow, ISignal, IDisposable, IDropTarget> >,
	public MenuProvider
{
private: // variables
...
public: // Object
	HWND Create(HWND hParent, PCTSTR name, PCTSTR classname, Direction dock = DirNone, DWORD dwStyle = 0, DWORD dwExStyle = 0)
	{
		...
		HWND hwnd = CWindowImplBaseT<WindowImplBase, CControlWinTraits>::Create(hParent, rcDefault, name, dwStyle, dwExStyle, 0U, atom, 0);
	...
	void __init__(IUnknown* arg)
	{
		...
		// 
		HWND hWnd = Create(hWndParent, name, classname, DirNone, WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, WS_EX_WINDOWEDGE);
		if(!hWnd)
...
public: // IDropTarget
	...
	ref<IDropTarget> QueryDropTargetToDescent(POINTL ptScreen)
	{
		...
		while(true)
		{
			...
			{	// hChild == null は、位置がクライアント領域外の場合に起こりうる.
...
namespace
{
	...
	void OnGestureUp(GestureDesc& desc, const MSG* msg)
	{
		if(!desc.gesture)
			return;
		::ReleaseCapture();
		if(FAILED(OnGestureFinish(desc)) && desc.history.size() == 1)
		{	// ジェスチャが無いので、通常の右クリック
			...
			case WM_XBUTTONUP:
				...
				desc.window.SendMessage(WM_XBUTTONDOWN, msg->wParam | (GET_XBUTTON_WPARAM(msg->wParam) == XBUTTON1 ? MK_XBUTTON1 : MK_XBUTTON2), lParamClient);
				break;
			}
		}
		EndGesture(desc);
	...
	{
		static const Gesture DIR[2][2] =
		{
			{ GestureNorth, GestureWest },
			{ GestureEast, GestureSouth }
		};
	...
	void OnGestureMouseMove(GestureDesc& desc, WPARAM, LPARAM lParam)
	{
		...
		if((dx*dx+dy*dy) < GESTURE_SKIP_DISTANCE*GESTURE_SKIP_DISTANCE)
		{	// 不感距離
			return;
		}
		...
		if(desc.history.empty() || desc.history.back() != dir || (dwTimeNow - desc.lastTime) > GESTURE_WAIT_TIME)
		{	// 新しいジェスチャ
	...
	inline ref<IGesture> QueryGestureToAncestor(HWND hwnd, const GestureDesc& desc)
	{
		...
		{
			ref<IGesture> gesture;
			if(QueryGestureInWindow(w, &gesture, desc.cursor, desc.history.size(), &desc.history[0])
			&& gesture
