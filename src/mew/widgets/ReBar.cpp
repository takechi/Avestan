...
public: // operation for implement
	void DrawBackground(HDC hDC, const Rect& rc, COLORREF clrBkgnd) const
	{
		...
		if(m_BackgroundImage)
		{
			...
			{
				if((m_WallPaperAlign & DirMaskWE) == (DirMaskWE))
				{	// tile both
			...
			else if((m_WallPaperAlign & DirMaskWE) == (DirMaskWE))
			{	// tile horz
				dc.FillSolidRect(&rc, clrBkgnd);
				for(int x = rc.left; x < rc.right; x += sz.w)
...
class ReBar : public WallPaperImpl< WindowImpl<
	CWindowImplEx< ReBar, CReBar< CReBarCtrlT<WindowImplBase> > >,
...
//			return true;
	...
	void DoCreate(CWindowEx parent)
	{
		m_locked = false;
		RECT rc = { 0, 0, 40, 40 };
		__super::DoCreate(parent, &rc, DirNorth, WS_CONTROL | CCS_NOPARENTALIGN | CCS_NOMOVEY | CCS_NODIVIDER | RBS_DBLCLKTOGGLE | RBS_VARHEIGHT | RBS_BANDBORDERS);
...
public: // IPersistMessage
	void LoadFromMessage(const message& msg)
	{
		...
		if(message archive = msg["bands"])
		{
			...
			if(m_locked)
			{	// バンドがひとつしかない場合にRBBS_NOGRIPPERを設定すると、自動的にRBBS_GRIPPERALWAYSも解除されてしまう。
