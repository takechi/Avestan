...
public:
	...
	HRESULT Get(REFGUID which, REFINTF what) const
	{
		...
		DoGet(hr, which, what, m_keymap)	||
		DoGet(hr, which, what, m_gesture)	||
		DoGet(hr, which, what, m_drop);
		return hr;
	}
	HRESULT Set(REFGUID which, IUnknown* what)
	{
		HRESULT hr = E_NOTIMPL;
		DoSet(hr, which, what, m_keymap)	||
		DoSet(hr, which, what, m_gesture)	||
		DoSet(hr, which, what, m_drop);
		return hr;
	}
	...
	bool ProcessQueryGesture(IGesture** pp)
	{
		return SUCCEEDED(m_gesture.copyto(pp));
	}
	bool ProcessQueryDrop(IDropTarget** pp)
	{
		return SUCCEEDED(m_drop.copyto(pp));
	}
};
