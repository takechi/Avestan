...
namespace
{
	...
	struct EntryCompare
	{
		...
		bool operator () (IEntry* lhs, IEntry* rhs) const
		{
			...
			if      ((short)HRESULT_CODE(hr) < 0)	{ /* lhs <  rhs */ return true; }
			else if ((short)HRESULT_CODE(hr) > 0)	{ /* lhs >  rhs */ return false; }
			else									{ /* lhs == rhs */ return false;  }
		}
	};
...
private:
	...
	void UpdateAttribute(DWORD dwAttr)
	{
		m_attrs = dwAttr;
	}
...
//			TRACE(_T("info: Remove Entry( $1 )"), this->Name);
	...
	static Entry* CreateEntryFromCSIDL(int csidl, PCWSTR path, PCWSTR next)
	{
		...
		parent->Release();
		if FAILED(hr)
			throw IOError(string::format(_T("$1 は無効なパスです"), path), STG_E_PATHNOTFOUND);
	...
	template < typename T > static T* __new__(IUnknown* arg)
	{
		if(!arg)
		{
			ITEMIDLIST* pidl = null;
		...
		else if(string path = cast(arg))
		{
			...
			catch(Error&)
			{
				...
				if(str::equals_nocase(wcsPath, wcsExpanded))
					throw;
...
public: // IEntry
	...
	HRESULT QueryObject(REFINTF ppObject, IndexOrIDList relpath = 0)
	{
		...
		{	// 自分自身
			...
			else if(ppObject.iid == IID_IContextMenu
				 || ppObject.iid == IID_IContextMenu2
				 || ppObject.iid == IID_IDropTarget
				 || ppObject.iid == IID_IDataObject
				 || ppObject.iid == IID_IExtractIcon
				 || ppObject.iid == IID_IQueryInfo)
			{
...
public: // IEntry
	...
	HRESULT ParseDisplayName(REFINTF ppObject, PCWSTR relpath)
	{
		return m_pInner->ParseDisplayName(ppObject, relpath);
	}
	bool Exists()
	{
		return m_pInner->Exists();
	}
	bool Equals(IEntry* rhs, NameType what)
	{
		if(!rhs)
	...
	ref<IEnumUnknown> EnumChildren(bool includeFiles)
	{
		return m_pInner->EnumChildren(includeFiles);
	}
};
...
public:
	...
	void __init__(IUnknown* arg)
	{
		...
		if(ref<IDataObject> data = cast(arg))
		{
			...
			if FAILED(hr)
				throw IOError(_T("IDataObject から CFSTR_SHELLIDLIST/TYMED_HGLOBAL を取得できません"), hr);
