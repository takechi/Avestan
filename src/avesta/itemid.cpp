...
HRESULT HresultFromShellExecute(HINSTANCE hInstance, PCWSTR path, PCWSTR verb, HWND hwnd) {
  if (hInstance > (HINSTANCE)32) return S_OK;
  if (hwnd) {  // hwnd が指定された場合のみエラーを表示
    ...
    if (msg) ::MessageBox(hwnd, msg.str(), null, MB_OK | MB_ICONERROR);
  }
...
HRESULT DoPathExecute(PCWSTR path, PCWSTR verb, PCWSTR args, PCWSTR dir, HWND hwnd) {
  ...
  if (!str::empty(verb)) {
    ...
    if (verb[0] == _T('.')) {                                                 // verb = 拡張子
      if FAILED (avesta::RegGetAssocExe(verb, exepath)) return E_INVALIDARG;  // 関連付けされたEXEを取得できなかった
      exe = exepath;
  ...
  if (str::empty(dir)) {  // ワーキングディレクトリは、起動するファイルと同じ
    str::copy(dirpath, path);
    PathRemoveFileSpec(dirpath);
    dir = dirpath;
  }

  if (hwnd) hwnd = ::GetAncestor(hwnd, GA_ROOTOWNER);
...
HRESULT DoILExecute(LPCITEMIDLIST pidl, PCWSTR path, PCWSTR verb, PCWSTR args, PCWSTR dir, HWND hwnd) {
  ...
  if (str::empty(dir) && !str::empty(path)) {  // ワーキングディレクトリは、起動するファイルと同じ
    str::copy(dirpath, path, MAX_PATH);
    PathRemoveFileSpec(dirpath);
    info.lpDirectory = dirpath;
  }
  if (ShellExecuteEx(&info)) return S_OK;
...
HRESULT avesta::PathExecute(PCWSTR path, PCWSTR verb, PCWSTR args, PCWSTR dir, HWND hwnd) {
  if (str::empty(path)) return E_INVALIDARG;
