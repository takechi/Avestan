...
namespace
{
	bool ChoiceBetter(LPTSTR bestfile, PCWSTR candidate)
	{
		...
		else
		{
			int i;
			for(i = 0;
				bestfile[i] != _T('\0') && candidate[i] != _T('\0')
				&& str::tolower(bestfile[i]) == str::tolower(candidate[i]);
				++i)
			{ }
...
HRESULT avesta::FileDup(PCWSTR src, PCWSTR dst)
{
	return DoFileOperation(src, dst, FO_COPY, FOF_ALLOWUNDO);
}

HRESULT avesta::FileMove(PCWSTR src, PCWSTR dst)
{
	return DoFileOperation(src, dst, FO_MOVE, FOF_ALLOWUNDO);
}

HRESULT avesta::FileDelete(PCWSTR src)
{
    return DoFileOperation(src, null, FO_DELETE, FOF_WANTNUKEWARNING | FOF_ALLOWUNDO);
}

HRESULT avesta::FileBury(PCWSTR src)
{
    return DoFileOperation(src, null, FO_DELETE, FOF_WANTNUKEWARNING);
}

HRESULT avesta::FileRename(PCWSTR src, PCWSTR dst)
{
	if(lstrcmp(src, dst) == 0)
	{	// 同じ名前なので変更する必要が無い
		return true;
	}
...
HRESULT avesta::FileCut(PCWSTR src)
{
	return FileCutOrCopy(src, DROPEFFECT_MOVE);
}

HRESULT avesta::FileCopy(PCWSTR src)
{
...
HRESULT avesta::FileCopy(Tuple* args)
{
	return FileCopyOrMove(args, FO_COPY);
}

HRESULT avesta::FileMove(Tuple* args)
{
	return FileCopyOrMove(args, FO_MOVE);
}

HRESULT avesta::FileDelete(Tuple* args)
{
	return FileDeleteOrBury(args, FOF_WANTNUKEWARNING | FOF_ALLOWUNDO);
}

HRESULT avesta::FileBury(Tuple* args)
{
	return FileDeleteOrBury(args, FOF_WANTNUKEWARNING);
}
