	...
	public:
		...
		int Loop(HWND hwnd)
		{
			...
			while(::IsWindow(hwnd))
			{
				...
				else if(m_needsUpdateUIState)
				{	// メッセージキューが空になったので、WM_UPDATEUISTATE をポストする。
		...
		{
			return (MessageHook*)::TlsGetValue(theThreadStateId);
		}
...
int Thread::Loop(HWND hwnd)
{
	return std::auto_ptr<MessageHook>(new MessageHook())->Loop(hwnd);
}
...
namespace
{
	int MessageLoop()
	{
		while(true)
		{
			...
			else if(WAIT_OBJECT_0 <= ret && ret < WAIT_OBJECT_0 + nCount)
			{	// スレッドのどれかが終了した
				RemoveThread(ret - WAIT_OBJECT_0);
			}
			else if(WAIT_ABANDONED_0 <= ret && ret < WAIT_ABANDONED_0 + nCount)
			{	// スレッドどれかが破棄済み？ ここには来ないかも。
				RemoveThread(ret - WAIT_ABANDONED_0);
			}
			else
			{	// エラー。既にスレッドが終了している可能性がある。
...
{
	if(MessageHook::Get()->GetMenuStackDepth() > 0)
		return true;
