...
void mew::theme::TabDrawHeader(HDC hDC, RECT& rc)
{
	...
	dc.SelectPen(penOld);
	rc.left += 2; rc.right -= 2; rc.top += 2; rc.bottom -= 1;
}

void mew::theme::TabDrawItem(HDC hDC, RECT bounds, PCWSTR text, DWORD status, HFONT hFontNormal, HFONT hFontBold, COLORREF clrActiveTab, COLORREF clrActiveText, COLORREF clrInactiveText)
{
	DCHandle dc(hDC);
	if(status & CHECKED)
	{
...
namespace
{
	...
	{
		return BlendRGB(::GetSysColor(a), ::GetSysColor(b), factor);
	}
	...
	static void DrawMenuIcon(DCHandle dc, bool selected, bool checked, bool disabled, IImageList* pImageList, IMAGELISTDRAWPARAMS& params)
	{
		ASSERT(pImageList);
...
void mew::theme::MenuDrawItem(HDC hDC, const RECT& rcBounds, DWORD dwStatus, PCWSTR wcsText, IImageList* pImageList, int nImage)
{
	DCHandle dc(hDC);
