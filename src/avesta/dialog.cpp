...
{
	MoveToCenter();
	m_hwndToolTip = ::CreateWindowEx(0, TOOLTIPS_CLASS, NULL, TTS_ALWAYSTIP, 0, 0, 0, 0, m_hwnd, NULL, (HINSTANCE)::GetWindowLongPtrW(m_hwnd, GWLP_HINSTANCE), NULL);
...
HWND Dialog::GetItem(UINT nID) const
{
	return ::GetDlgItem(m_hwnd, nID);
}

int Dialog::GetText(UINT nID, PTSTR text, int nMaxCount) const
{
	return ::GetDlgItemText(m_hwnd, nID, text, nMaxCount);
}

void Dialog::SetText(UINT nID, PCTSTR text)
{
	::SetDlgItemText(m_hwnd, nID, text);
}

bool Dialog::GetEnabled(UINT nID) const
{
	return ::IsWindowEnabled(GetItem(nID)) != 0;
}

void Dialog::SetEnabled(UINT nID, bool enable)
{
	::EnableWindow(GetItem(nID), enable);
}

bool Dialog::GetChecked(UINT nID) const
{	// BST_CHECKED, BST_INDETERMINATE, BST_UNCHECKED
	return ::SendMessage(GetItem(nID), BM_GETCHECK, 0, 0) == BST_CHECKED;
}

void Dialog::SetChecked(UINT nID, bool check)
{	// BST_CHECKED, BST_INDETERMINATE, BST_UNCHECKED
