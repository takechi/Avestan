...
HRESULT RegGetAssocCommand(PCWSTR key, WCHAR command[MAX_PATH]) {
  ...
  if (str::empty(key)) return AtlHresultFromWin32(ERROR_REGISTRY_IO_FAILED);
  // デフォルトコマンドを取得
  wsprintf(buffer, _T("%s\\shell"), key);
  if (FAILED(RegGetString(HKEY_CLASSES_ROOT, buffer, NULL, def)) || str::empty(def)) lstrcpy(def, _T("open"));
  // コマンドの内容を取得
  wsprintf(buffer, _T("%s\\shell\\%s\\command"), key, def);
  if FAILED (hr = RegGetString(HKEY_CLASSES_ROOT, buffer, NULL, command)) return hr;
  if (str::empty(command)) return AtlHresultFromWin32(ERROR_REGISTRY_IO_FAILED);
  return S_OK;
}
}  // namespace
