...
namespace
{
	...
	public:
		AttributesImpl(MSXML2::ISAXAttributes* attr) : m_pAttr(attr)
		{
			ASSERT(m_pAttr);
		}
...
#define TO_STRING0(chars)		(PCWSTR)chars
	...
	public:
		...
		STDMETHODIMP putDocumentLocator( 
			MSXML2::ISAXLocator* pLocator)
		{ return S_OK; }
			
		STDMETHODIMP startDocument()
		{ return m_pHandler->StartDocument(); }
		
		STDMETHODIMP endDocument()
		{ return m_pHandler->EndDocument(); }

		STDMETHODIMP startPrefixMapping( 
			XMLSTR pwchPrefix,	int cchPrefix,
			XMLSTR pwchUri,	int cchUri)
		{ return S_OK; }
		...
		STDMETHODIMP startElement( 
			XMLSTR pwchNamespaceUri,	int cchNamespaceUri,
			XMLSTR pwchLocalName,	int cchLocalName,
		...
		STDMETHODIMP characters( 
			XMLSTR pwchChars,	int cchChars)
		{ return m_pHandler->Characters(TO_STRING(pwchChars, cchChars)); }
		...
		STDMETHODIMP processingInstruction( 
			XMLSTR pwchTarget,	int cchTarget,
			XMLSTR pwchData,	int cchData)
		...
		STDMETHODIMP skippedEntity( 
			XMLSTR pwchName,	int cchName)
		{ return S_OK; }

		STDMETHODIMP error( 
			MSXML2::ISAXLocator* pLocator,
			XMLSTR pwchErrorMessage,
			HRESULT errCode)
		{
			int line, column;
...
public: // Object
	...
	{
		ASSERT(!arg);
		if SUCCEEDED(::CoCreateInstance(__uuidof(SAXXMLReader40), null, CLSCTX_INPROC, __uuidof(MSXML2::ISAXXMLReader), (void**)&m_pNativeReader))
		{
