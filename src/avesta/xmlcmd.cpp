...
ref<ICommand> avesta::XmlAttrCommand(XMLAttributes& attr, ICommands* commands) {
  ...
  if (string command = attr[ATTR_COMMAND]) return CreateBuiltInCommand(commands, command, args);

  // @execute => Execute command
  if (string execute = attr[ATTR_EXECUTE]) return CreateExecuteCommand(execute, args);

  // @navigate => Navigate command
  if (string navigate = attr[ATTR_NAVIGATE]) return CreateNavigateCommand(navigate, args);
...
ref<IEditableTreeItem> avesta::XmlAttrTreeItem(XMLAttributes& attr, ICommands* commands) {
  ...
  if (string command = attr[ATTR_COMMAND]) item = CreateSpecialMenu(command);  // 特殊メニューの作成を試みる
  if (!item) {                                                                 // 一般メニュー
    item.create(__uuidof(DefaultTreeItem));
    if (ref<ICommand> command = XmlAttrCommand(attr, commands)) item->Command = command;
  }
  //
