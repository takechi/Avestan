...
private:
	HRESULT TaskTray_HandleResize(message msg)
	{
		...
		if(!IsWindowEnabled(hwnd))
			return S_OK;
		if(m_EnableTray && ::IsIconic(hwnd) && m_form->Visible)
		{	// 復帰は、Form.CommandRestore で実装されているため処理する必要は無い
	...
	{
		if(m_form && m_CloseToTray && m_form->Visible && theMainResult == 0)
		{	// cancel close and minimize to tasktray
