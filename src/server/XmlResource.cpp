...
namespace
{
	...
	inline static bool equals(PCWSTR lhs, PCWSTR rhs, size_t cch)
	{
		return wcsncmp(lhs, rhs, cch) == 0 && wcslen(rhs) == cch;
	}
}
	...
	public:
		...
		void Invoke()
		{
			...
			if FAILED(theAvesta->GetComponent(&owner, AvestaFolder))
				if FAILED(theAvesta->GetComponent(&owner, AvestaForm))
					return;
			...
			Point pt = rc.center;
			// FIXME: ugly...
			if(ref<ICommand> cmd = mew::ui::MenuProvider::PopupMenu(m_menu, ::GetAncestor(hwnd, GA_ROOT), TPM_RIGHTBUTTON | TPM_CENTERALIGN | TPM_VCENTERALIGN, pt.x, pt.y))
				cmd->Invoke();
		}
	};

	ref<ICommand> CommandFromMenu(ITreeItem* menu)
	{
		return ref<ICommand>::from(new PopupCommand(menu));
	}
}
...
namespace
{
	...
	protected:
		...
		void PushTreeItem(IEditableTreeItem* item)
		{
			...
			{	// ルートレベルに複数のメニューが並んでいる。ダミールートを挿入する。
	...
	public:
		...
		HRESULT EndElement(PCWSTR name, size_t cch)
		{
			if(equals(name, KEYMAP_BIND, cch))
			{
				if(m_vkey != 0)
				{
					if(ref<ITreeItem> menu = DetachTreeItem())
...
namespace
{
	...
	public: // ITreeItem
		...
		void set_Name(string value)			{ m_Text = value; ++m_TimeStamp; }
		...
		{
			return m_Children.size() + m_pFolder->GetChildCount();
		}
		...
		UINT32 OnUpdate()
		{
			return m_TimeStamp = math::max(m_TimeStamp, m_pFolder->OnUpdate());
		}
	...
	public:
		LinkReader(function fn) : m_invokeHandler(fn)
		{
			m_FolderImageIndex = afx::ExpGetImageIndexForFolder();
		}
		~LinkReader()
		{
		}
		HRESULT StartElement(PCWSTR name, size_t cch, XMLAttributes& attr)
		{
			if(equals(name, ITEM_NODE, cch))
			{
				...
				{
					includeFiles = (includeFiles_s.equals_nocase(_T("true")) || includeFiles_s.equals_nocase(_T("yes")) || str::atoi(includeFiles_s.str()) != 0);
				}
				ref<IFolder> folder;
				if(path)
				{
