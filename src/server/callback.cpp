...
namespace ave
{
	...
	MEW_API UINT64 GetTotalBytes(IShellListView* view)
	{
		...
		if FAILED(hr = view->GetContents(&entries, StatusNone))
			return hr;
	...
	MEW_API UINT64 GetSelectedBytes(IShellListView* view)
	{
		...
		HRESULT hr;
		ref<IEntryList> entries;
		if FAILED(hr = view->GetContents(&entries, SELECTED))
			return hr;
		ref<IShellFolder> folder;
		if FAILED(hr = view->GetFolder(&folder))
			return hr;
...
public:
	...
	string StatusText(const string& text, IShellListView* view)
	{
		...
		{	// 選択なし
			...
			else
			{
				...
				if(totalCount > 0)
				{	// totalCount == 0 の場合、まだフォルダを計算中の可能性があり、
	...
	string GestureText(const Gesture gesture[], size_t length, const string& description)
	{
		...
		{
			_T("左"), _T("右"), _T("中"), _T("４"), _T("５"), _T("△"), _T("▽"),
			_T("←"), _T("→"), _T("↑"), _T("↓"), 
		};
		StringBuffer text;
		text.reserve(64);
