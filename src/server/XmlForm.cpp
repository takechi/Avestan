...
namespace
{
	...
	//		t.name		= attr[L"name"];
		...
		HRESULT EndElement(PCWSTR name, size_t cch)
		{
			...
			if(m_forms[current].clsid == GUID_NULL)
			{	// unknown class name
...
HRESULT FormGenerate(Templates& forms)
{
	...
	for(int i = 0; i < (int)forms.size(); ++i)
	{
		Template& t = forms[i];
		//
		     if(t.type == FORMTYPE_FORM ) indexForm  = i;
		else if(t.type == FORMTYPE_PANEL) indexPanel = i;
		else if(t.type == FORMTYPE_MAIN ) indexMain  = i;
		//
		ref<IWindow> parent;
		if(t.parent >= 0)
...
namespace
{
	...
	ref<T> LoadResource(
		string relname,
		ICommands* commands, IXMLReader* sax,
		ref<T> (*fn)(string, ICommands*, IXMLReader*), UINT errmsg)
	{
		...
		if(ref<T> res = fn(path, commands, sax))
			return res;
		// error
	...
	ref<ITreeItem> LoadLinkTree(string relname, function onOpen, IXMLReader* sax)
	{
		...
		ref<ITreeItem> root;
		if(PathIsDirectory(xml))
		{	// link from directory
			return LoadLinkTreeFromDirectory(xml, onOpen);
		}
		else if(PathFileExists(xml))
		{	// link from xml
	...
	void ReloadForm(Template& t, ICommands* commands, IXMLReader* sax)
	{
		...
		if(ref<ITree> tree = cast(t.window))
		{
			...
			if(t.item)
			{
				tree->Root = (t.type == L"LinkBar") ?
					LoadLinkTree(t.item, OnOpenEntry, sax) :
...
HRESULT	FormComponentsHide(Templates& forms)
{
	for(Templates::iterator i = forms.begin(); i != forms.end(); ++i)
	{
		if(!i->window)
			continue;
		if(i->type == FORMTYPE_FORM
		|| i->type == FORMTYPE_PANEL
		|| i->type == FORMTYPE_MAIN)
		{
		}
		else
		{
