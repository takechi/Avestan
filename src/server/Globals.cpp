...
Avesta::~Avesta() {
  for (size_t i = 0; i < NumOfFonts; ++i) {
    if (m_fonts[i]) ::DeleteObject(m_fonts[i]);
  }
}
...
void Avesta::LoadFromMessage(message& msg) {
  ...
  for (size_t i = 0; i < NumOfFonts; ++i) {
    if (message m = msg[FONTS[i].name]) m_fonts[i] = LoadFont(m);
  }
}
...
void Avesta::SaveToMessage(message& msg) {
  ...
  for (size_t i = 0; i < NumOfFonts; ++i) {
    if (message m = SaveFont(m_fonts[i])) msg[FONTS[i].name] = m;
  }
}

HFONT Avesta::FontFromMessage(Fonts what, message& msg) {
  if (HFONT hFont = (HFONT)(INT_PTR)msg["font"]) {
    if (m_fonts[what]) ::DeleteObject(m_fonts[what]);
    m_fonts[what] = hFont;
  }
  return m_fonts[what];
}
...
void UpdateDefaultEffect(IEntry* dst) {
  if (!dst || !avesta::GetOption(BoolDnDCopyInterDrv)) {  // dst が指定されていない場合は変更しない
    return;
  } else if (str::empty(theSourcePath)) {  // ソースパスが空……よくわからんのでデフォルトを設定しない
    theDefaultEffect = 0;
  } else {
...
HRESULT mew::ui::ProcessDragEnter(IDataObject* src, IEntry* dst, DWORD key, DWORD* effect) {
  if (avesta::GetOption(BoolDnDCopyInterDrv)) {
    try {
      ...
      if FAILED (afx::ILGetPath(entries->Parent, theSourcePath)) str::clear(theSourcePath);
    } catch (Error& e) {
      TRACE(e.Message);
    }
  }
...
HRESULT DropToExe(IDataObject* src, IEntry* dst) {
  string exe = dst->Path;
  if (!PathIsExe(exe.str())) return E_FAIL;
  ref<IEntryList> entries(__uuidof(EntryList), src);
...
static HRESULT EnumShellWindow(HWND hwnd, LPCITEMIDLIST pidl, LPARAM lParam) {
  ...
  if (SUCCEEDED(CreateEntry(&folder, pidl)) && folder->IsFolder()) {
    ...
    if (imp->close) ::PostMessage(hwnd, WM_CLOSE, 0, 0);
  }
  return S_OK;
}
}  // namespace
