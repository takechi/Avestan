...
namespace
{
	...
	public:
		...
		void AutoNumbering(PCTSTR text)
		{	// 自分好みのリネームルーチン。他の人が使いやすいかどうかは知らない。
			size_t selectCount = m_src.size();
			int n = 0;
			for(size_t cnt = selectCount; cnt > 0; cnt /= 10) { ++n; }
		...
		static void GetLocation(PCTSTR text, int* index, int* beg, int* end)
		{
			...
			{
				PCTSTR num = bra+1;
				while(*num != '\0' && !str::find(_T("0123456789>"), *num)) { ++num; }
				*index = str::atoi(num);
				*beg = bra-text;
		...
		void ReplaceAll(HWND hListView, HWND hEdit, bool preserveExtension)
		{
			...
			int width = end-beg-1;
			int n = 0;
			for(size_t cnt = count; cnt > 0; cnt /= 10) { ++n; }
	...
	private:
		...
		bool EndLabelEdit(HWND hListView, NMLVDISPINFOW* nm)
		{
			...
			if(!text || index >= m_dst.size())
				return false;
			m_dst[index] = text;
			if(index+1 < m_dst.size())
			{	// 次のアイテムを編集
...
void DlgRename(IShellListView* view, bool paste)
{
	...
	if FAILED(view->GetFolder(&folder))
		return;
	if FAILED(view->GetContents(&entries, SELECTED))
		return;
