...
class CommandList : public Root<implements<ICommand>, mixin<StaticLife> > {
 ...
 public:
  ...
  HRESULT GetAt(REFINTF ppChild, size_t index) const {
    if (index >= m_queue.size()) return E_INVALIDARG;
...
class MRUList : public CommandList {
 ...
 public:
  ...
  void Push(IEntry* entry) {
    ...
    if (m_queue.size() > MAX_ENTRY_COUNT) m_queue.resize(MAX_ENTRY_COUNT);
    Renumbering();
  }
  ref<IEntry> Pop() {
  ...
  string get_Description() { return _T("最後に閉じられたフォルダを開きます"); }
