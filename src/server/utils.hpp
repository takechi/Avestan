...
inline ref<IEntry> GetFolderOfView(IShellListView* view) {
  if (!view) return null;
  ref<IEntry> folder;
  if FAILED (view->GetFolder(&folder)) return null;
  return folder;
}
...
inline string GetPathOfView(IShellListView* view) { return GetNameOfView(view, IEntry::PATH); }
...
struct WindowRef {
  ...
  template <class T>
  WindowRef(const ref<T>& window) : hWnd(window ? window->Handle : 0) {}
...
inline int MessageBox(HWND hWnd, PCTSTR text, UINT type = MB_OK) {
  ...
  if (hParent) ::GetWindowText(hParent, caption, MAX_PATH);
