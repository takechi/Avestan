...
namespace mew
{
	...
	__interface ISignal : IDisposable
	{
		...
		HRESULT Connect(
			EventCode code,      ///< 登録するメッセージコード. サポートしていないメッセージコードの場合は呼び出しが失敗する.
		...
		/// @result 削除されたシンクの個数.
		size_t Disconnect(
			EventCode code,      ///< 削除するメッセージコード. 0の場合は全てのコードが削除される.
	...
	/// 
	template < int code > struct Event
	{
		template < class T > static void event(message& msg, T* from)
		{
			msg["from"] = from;
		}
	};
