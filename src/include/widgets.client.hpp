...
class CWindowEx : public ATL::CWindow {
 ...
 public:
  ...
  CWindowEx GetAncestor(UINT gaFlags) const { return ::GetAncestor(m_hWnd, gaFlags); }
  bool HasFocus() const {
...
#define CHAIN_MSG_MAP_TO(name)                                  \
  {                                                             \
    if (name(hWnd, uMsg, wParam, lParam, lResult)) return true; \
  }

// bool OnMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lResult)
#define MSG_HANDLER(msg, func)                                  \
  if (uMsg == msg) {                                            \
    if ((bool)func(uMsg, wParam, lParam, lResult)) return true; \
  }
