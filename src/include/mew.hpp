...
template <class T>
class ref;
using string = ref<IString>;    ///< 文字列. 実際には参照だが、immutable
                                ///< のため値型だと考えても構わない.
...
#define TRESPASS()                                                    \
  if (mew::Assert(L"TRESPASS", __FILE_W__, __LINE__, __FUNCTION_W__)) \
    ;                                                                 \
  else                                                                \
    __debugbreak()
...
#define ASSERT(exp)                                                  \
  if (exp)                                                           \
    ;                                                                \
  else if (mew::Assert(L#exp, __FILE_W__, __LINE__, __FUNCTION_W__)) \
    ;                                                                \
  else                                                               \
    __debugbreak()
#define VERIFY(exp) ASSERT(exp)
#define ASSERT_HRESULT(exp)                                          \
  if (SUCCEEDED(exp))                                                \
    ;                                                                \
  else if (mew::Assert(L#exp, __FILE_W__, __LINE__, __FUNCTION_W__)) \
    ;                                                                \
...
struct REFINTF {
  ...
  void** const pp;
  template <class T>
  REFINTF(T** ppT) : pp((void**)ppT), iid(__uuidof(T)) {
    ASSERT(pp);
  }
  template <class T>
  REFINTF(REFIID riid, T** ppT) : pp((void**)ppT), iid(riid) {
    ASSERT(pp);
  }
};
...
struct POD {
  friend IStream& operator<<(IStream& stream, const POD& pod) {
    io::StreamWriteExact(&stream, static_cast<const T*>(&pod), sizeof(T));
    return stream;
  }
  friend IStream& operator>>(IStream& stream, POD& pod) {
    io::StreamReadExact(&stream, static_cast<T*>(&pod), sizeof(T));
    return stream;
  }
};
...
/// オブジェクトを生成する.
MEW_API void CreateInstance(REFCLSID clsid,       ///< クラスID.
