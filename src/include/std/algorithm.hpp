...
namespace mew
{
	...
	namespace algorithm
	{
		...
		/// binary_search
		/// std::binary_search() は bool値しか返さないのに対し、これは見つかったイタレータを返す.
		...
		ptr_method_1_t<Obj, Result (T::*)(Arg), Result, Arg>
...
*/
		...
		std::binder2nd< std::mem_fun1_t<Result, T, Arg> >
		mem_fun(Result (T::*m)(Arg), Obj obj)
		{
		...
		/// obj->method(_1)
		template < class Obj, class T, class Result, class Arg >
		std::binder1st< std::mem_fun1_ref_t<Result, T, Arg> >
		mem_fun_ref(Obj obj, Result (T::*m)(Arg))
		{
		...
		/// _1->method(obj)
		template < class T, class Result, class Arg, class Obj >
		std::binder2nd< std::mem_fun1_ref_t<Result, T, Arg> >
		mem_fun_ref(Result (T::*m)(Arg), Obj obj)
		{
