...
class array_set {
 ...
 public:
  explicit array_set(const Pred& pred = Pred(), const Alloc& al = Alloc()) : key_compare_(pred), vec_(al) {}
  template <class It>
  array_set(It first, It beyond, const Pred& pred = Pred(), const Alloc& al = Alloc())
      : key_compare_(pred), vec_(first, beyond, al) {
    stable_sort();
  }
  array_set(const self_type& x) : vec_(x.vec_), key_compare_(x.key_compare_) {}
  ~array_set() {}
  ...
  self_type& operator=(const Cont& x) {
    vec_.operator=(x);
    sort();
    return *this;
  }
  ...
  iterator insert(iterator it, const value_type& hint) {
    if (it != end()) {
      if (Unique) {
        ...
        if (KeyCompare_Leq_(*it, hint) && ((it + 1) == end() || KeyCompare_Geq_(*(it + 1), hint))) {
          return InsertImpl_(it + 1, hint);
        }
      }
    }
  ...
  bool Eq_(const self_type& x) const { return (size() == x.size() && std::equal(begin(), end(), x.begin())); }
  bool Lt_(const self_type& x) const { return (std::lexicographical_compare(begin(), end(), x.begin(), x.end())); }
  void swap(self_type& x) {
    vec_.swap(x.vec_);
    std::swap(key_compare_, x.key_compare_);
  }

  friend void swap(self_type& x, self_type& Y_) { x.swap(Y_); }
  ...
  template <class U>
  iterator lower_bound(const U& k) {
    return std::lower_bound(begin(), end(), k, key_compare_);
  }
  template <class U>
  const_iterator lower_bound(const U& k) const {
    return std::lower_bound(begin(), end(), k, key_compare_);
  }
  template <class U>
  iterator upper_bound(const U& k) {
    return std::upper_bound(begin(), end(), k, key_compare_);
  }
  template <class U>
  const_iterator upper_bound(const U& k) const {
    return std::upper_bound(begin(), end(), k, key_compare_);
  }
  template <class U>
  Pairii_ equal_range(const U& k) {
    return std::equal_range(begin(), end(), k, key_compare_);
  }
  template <class U>
  Paircc_ equal_range(const U& k) const {
    return std::equal_range(begin(), end(), k, key_compare_);
  }
 ...
 protected:
  ...
  iterator InsertImpl_(iterator p, const value_type& x) { return vec_.insert(p, x); }
  bool KeyCompare_Leq_(const T& ty0, const T& ty1) { return !key_compare_(ty1, ty0); }
  bool KeyCompare_Geq_(const T& ty0, const T& ty1) { return !key_compare_(ty0, ty1); }
  bool KeyCompare_Gt_(const T& ty0, const T& ty1) { return key_compare_(ty1, ty0); }

  key_compare key_compare_;
  Cont vec_;
};

template <class T, bool Unique, class Pred, class Alloc>
inline bool operator==(const array_set<T, Unique, Pred, Alloc>& x, const array_set<T, Unique, Pred, Alloc>& Y_) {
  return x.Eq_(Y_);
}
template <class T, bool Unique, class Pred, class Alloc>
inline bool operator!=(const array_set<T, Unique, Pred, Alloc>& x, const array_set<T, Unique, Pred, Alloc>& Y_) {
