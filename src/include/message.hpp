...
class variant {
 ...
 private:
  ...
  struct Get_;  // friend struct Get_;
  template <typename T, bool isPOD>
  struct Set_;  // friend struct Set_;
  template <typename T>
  struct Get_<T, true> {
    enum { Code = VariantType<T>::Code };
    using Result = typename VariantResult<T>::Result;
    static Result get(const variant* self) {
      Result result;
      self->ToPOD(Code, &result, sizeof(result));
      return result;
    }
  };
  template <typename T>
  ...
  template <typename T>
  struct Get : Get_<T, IsPOD<T>::value> {};
  template <typename T>
  struct Set : Set_<T, IsPOD<T>::value> {};
  template <>
  struct Set<function> {
    static void set(variant* self, const function& v) { self->FromFunction(v); }
  };
  template <>
  struct Get<Null> {};  // invalid
  template <>
  struct Set<Null> {};  // invalid
  template <>
  struct Get<variant> {};  // invalid
  template <>
  struct Set<variant> {};  // invalid
 ...
 public:
  ...
  // get & operator cast
  template <typename T>
  typename Get<T>::Result get() const {
 ...
 private:
  ...
  friend IStream& operator<<(IStream& stream, const variant& v) {
    v.save(stream);
    return stream;
  }
};
...
/// メッセージを作成する.
void CreateMessage(IMessage** ppMessage,  ///< 作成されたメッセージ.
...
class ref<IMessage> : public ref_base<IMessage> {
  ...
  class proxy {
   ...
   public:
    ...
    void operator=(const T& rhs) {
      m_owner.DoSet(m_key, variant(rhs));
    }
    template <typename T>
    const proxy<ref> operator[](const T& key) const {
      return const_cast<proxy*>(this)->operator[](key);
    }
    template <typename T>
    proxy<ref> operator[](const T& key) {
      ref msg = get();
      ASSERT(msg);
 ...
 public:
  ...
  template <typename T>
  const proxy<ref&> get(const T& key) const {
    return const_cast<ref*>(this)->get(key);
  }
  template <typename T>
  proxy<ref&> get(const T& key) {
    return proxy<ref&>(*this, Guid(key));
  }
  template <typename T>
  const proxy<ref&> operator[](const T& key) const {
    return get(key);
...
class function {
 public:
  ...
  using instance_function_t = HRESULT (TClass::*)(message);
...
template <>
struct VariantType<INT8> {
  enum { Code = TypeSint8 };
};
template <>
struct VariantType<UINT8> {
  enum { Code = TypeUint8 };
};
template <>
struct VariantType<INT16> {
  enum { Code = TypeSint16 };
};
template <>
struct VariantType<UINT16> {
  enum { Code = TypeUint16 };
};
template <>
struct VariantType<INT32> {
  enum { Code = TypeSint32 };
};
template <>
struct VariantType<UINT32> {
  enum { Code = TypeUint32 };
};
template <>
struct VariantType<INT64> {
  enum { Code = TypeSint64 };
};
template <>
struct VariantType<UINT64> {
  enum { Code = TypeUint64 };
};
template <>
...
template <typename T>
struct VariantType<ref<T> > : VariantType<IUnknown*> {};
template <>
struct VariantType<const wchar_t*> : VariantType<wchar_t*> {};
template <size_t sz>
struct VariantType<wchar_t[sz]> : VariantType<wchar_t*> {};
template <size_t sz>
struct VariantType<const wchar_t[sz]> : VariantType<wchar_t*> {};

//==============================================================================

template <typename T>
struct VariantResult<T*> {
  using Result = ref<T>;
};
template <typename T>
struct VariantResult<ref<T> > {
  using Result = ref<T>;
};
template <typename T>
struct VariantResult<ref_base<T> > {
  using Result = ref_base<T>;
};
}  // namespace mew
