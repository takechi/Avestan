...
interface __declspec(novtable) IList : IWindow {
  ...
  /// アイテムの状態を取得する.
  virtual HRESULT GetStatus(IndexOr<IUnknown> item,  ///< 対象のアイテムまたはインデックス.
                            DWORD * status,          ///< アイテムの状態. nullの場合は取得しない.
                            size_t* index = null     ///< アイテムのインデックス. nullの場合は取得しない.
                            ) = 0;
  /// アイテムの状態を設定する.
  virtual HRESULT SetStatus(IndexOr<IUnknown> item,  ///< 対象のアイテムまたはインデックス.
...
interface __declspec(novtable) ITreeView : ITree {
  ...
  virtual HRESULT GetContents(REFINTF ppInterface, Status status) = 0;
  /// アイテムの状態を取得する.
  virtual HRESULT GetStatus(IUnknown * item,  ///< 対象のアイテム.
                            DWORD * status    ///< アイテムの状態. nullの場合は取得しない.
                            ) = 0;
  /// アイテムの状態を設定する.
  virtual HRESULT SetStatus(IUnknown * item,  ///< 対象のアイテム.
                            Status status     ///< アイテムの状態.
                            ) = 0;
};
...
interface __declspec(novtable) IShellListView : IListView {
  ...
  __declspec(
      property(get = get_RenameExtension, put = set_RenameExtension)) bool RenameExtension;  ///< リネーム時に拡張子を選択する.
  __declspec(property(get = get_PatternMask, put = set_PatternMask)) string PatternMask;  ///< フィルタ文字列

  /// フォルダを移動する.
  virtual HRESULT Go(io::IEntry * folder  ///< 移動先のフォルダ.
                     ) = 0;
  /// フォルダを移動する.
  virtual HRESULT Go(Direction dir,  ///< 方向. DirWest=戻る, DirEast=進む, DirNorth=進む.
                     int level = 1   ///< 段階.
                     ) = 0;
  /// 現在のフォルダを取得する.
  virtual HRESULT GetFolder(REFINTF ppFolder  ///< io::IEntry または IShellFolder を取得できる.
                            ) = 0;
...
inline bool IsKeyPressed(int vkey) { return (::GetKeyState(vkey) & 0x8000) != 0; }
...
inline HRESULT QueryParent(HWND hWnd, REFINTF pp) { return QueryInterfaceInWindow(::GetAncestor(hWnd, GA_PARENT), pp); }
inline HRESULT QueryParent(IWindow* window, REFINTF pp) {
  if (!window) return E_POINTER;
  return QueryParent(window->Handle, pp);
}

inline HRESULT QueryParentOrOwner(HWND hWnd, REFINTF pp) { return QueryInterfaceInWindow(afx::GetParentOrOwner(hWnd), pp); }
}  // namespace ui
}  // namespace mew
