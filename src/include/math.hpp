...
namespace mew
{
	...
	//	const REAL64 PI	= 3.14159265358979323846;	///< â~é¸ó¶(REAL64).
		...
		template < typename T > T clamp(T v, T min_, T max_)	{ return min(max(v, min_), max_); }

		template < typename T > T mod(T x, T y)			{ return x % y; }
		template < typename T > T sqr(T x)				{ return x * x; }
		template < typename T > T hypot2(T x, T y)		{ return x * x + y * y; }

		//==============================================================================

		template <> inline int		abs<int>(int x)			{ return std::abs(x); }
		template <> inline long		abs<long>(long x)		{ return std::abs(x); }
		template <> inline REAL32	abs<REAL32>(REAL32 x)	{ return std::fabsf(x); }
		template <> inline REAL64	abs<REAL64>(REAL64 x)	{ return std::fabs(x); }
		template <> inline __int64	abs<__int64>(__int64 x)	{ return _abs64(x); }

		template <> inline REAL32 mod<REAL32>(REAL32 x, REAL32 y)	{ return std::fmodf(x, y); }
		template <> inline REAL64 mod<REAL64>(REAL64 x, REAL64 y)	{ return std::fmod(x, y); }
