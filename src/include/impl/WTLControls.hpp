...
namespace WTLEX
{
	...
	template < class TParam, class TItem > struct CTypedItem : TItem
	{
	...
	public:
		...
		{
			return (ParamType)__super::GetItemData(nItem);
		}
		...
		INT32 GetSelectedColumn() const throw()
		{
	...
	public: // operation
		...
		{
			if(column < 0 || column >= GetColumnCount())
			{	// 不正なので、デフォルト (0, ascending) に設定する
				column = 0;
				if(flags == SortDefault)
					flags = SortAscending;
			}
			bool ascending;
			switch(flags)
	...
	public:
		ParamType GetItemData(HTREEITEM hItem) const
		{
			return hItem ? (ParamType)__super::GetItemData(hItem) : ParamType();
		}
		HTREEITEM HitTest(TVHITTESTINFO* info) const
		{
			return const_cast<CTypedTree*>(this)->__super::HitTest(info);
		}
		...
		{
			__super::SetItem(const_cast<Item*>(&item));
		}
		...
		template < class Op > bool ForEach(HTREEITEM hParent, Op op)
		{
	...
	public:
		...
		{	// ツリービューではなく、デフォルト（WM_CONTEXTMENUを発生させる）
			::DefWindowProc(m_hWnd, uMsg, wParam, lParam);
			return 0;
		}
		LRESULT OnItemReturn(int, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
		{	// デフォルト動作は「ビープを鳴らす」ので、キャンセルする。
			return 1;
		}
		LRESULT OnItemKeyDown(int, LPNMHDR pnmh, BOOL& bHandled)
		{	// NM_RETURN では、Ctrl+Return を取得できないため、TVN_KEYDOWN をハンドルする。
