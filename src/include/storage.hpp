...
namespace mew
{
	namespace io
	{
		...
		public:
			...
			HRESULT __stdcall CopyTo(IStream* pStream, ULARGE_INTEGER size, ULARGE_INTEGER* read, ULARGE_INTEGER* written)	{ return E_NOTIMPL; }
		...
		private:
			struct Mapped
			{
				...
				friend inline IStream& operator << (IStream& stream, const Mapped& v) throw(...)
				{
					return stream << v.value;
				}
			};
