...
class ref<IStream> : public ref_base<IStream> {
 ...
 public:
  ...
  bool read(void* buffer, size_t size, size_t& done) const {
    done = io::StreamReadSome(m_ptr, buffer, size);
    return done > 0;
  }
  const Stream& read(void* buffer, size_t size) const {
    io::StreamReadExact(m_ptr, buffer, size);
    return *this;
  }
  const Stream& read(REFINTF obj) const {
    io::StreamReadObject(m_ptr, obj);
    return *this;
  }
  bool write(const void* buffer, size_t size, size_t& done) const {
    done = io::StreamWriteSome(m_ptr, buffer, size);
    return done > 0;
  }
  const Stream& write(const void* buffer, size_t size) const {
    io::StreamWriteExact(m_ptr, buffer, size);
    return *this;
  }
  const Stream& write(IUnknown* obj) const {
    io::StreamWriteObject(m_ptr, obj);
    return *this;
  }
  void seek(INT64 mov, UINT64* newpos = null) const { io::StreamSeekRel(m_ptr, mov, newpos); }
  UINT64 get_position() const {
    UINT64 newpos;
    io::StreamSeekRel(m_ptr, 0, &newpos);
    return newpos;
  }
  ...
  template <typename T>
  friend const Stream& operator>>(const Stream& stream, ref<T>& ptr) {
    stream.read(&ptr);
    return stream;
  }
  friend const Stream& operator<<(const Stream& stream, IUnknown* ptr) {
    stream.write(ptr);
    return stream;
  }
...
#define AVESTA_POD(POD)                                         \
  ...
  inline IStream& operator>>(IStream& stream, POD& pod) {       \
    mew::io::StreamReadExact(&stream, &pod, sizeof(POD));       \
    return stream;                                              \
  }                                                             \
  template <>                                                   \
  struct mew::IsPOD<POD> {                                      \
    enum { value = 1 };                                         \
  };
...
inline IStream& operator<<(IStream& stream, const mew::POD<T>& pod) {
  mew::io::StreamWriteExact(&stream, static_cast<const T*>(&pod), sizeof(T));
  return stream;
}

template <typename T>
inline IStream& operator>>(IStream& stream, mew::POD<T>& pod) {
  mew::io::StreamReadExact(&stream, static_cast<T*>(&pod), sizeof(T));
  return stream;
}
