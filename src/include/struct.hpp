...
#undef MEW_TUPLE_ARG
	...
	struct Size : SIZE, Tuple<Size, 2>
	{
		...
		void assign(INT32 ww, INT32 hh)	{ cx = ww; cy = hh; }
	...
	struct Point : POINT, Tuple<Point, 2>
	{
		...
		void assign(INT32 xx, INT32 yy)	{ x = xx; y = yy; }
		void assign(const Point& rhs)		{ x = rhs.x; y = rhs.y; }
	...
	struct Rect : RECT
	{
		...
		void assign(INT32 l, INT32 t, INT32 r, INT32 b)	{ left = l; top = t; right = r; bottom = b; }
		void assign(const Point& pt, const Size& sz)		{ location = pt; size = sz; }
	...
	*/
		...
		const INT32 set_x(INT32 value)	{ right  += value - left; left = value; return value; }
		const INT32 set_y(INT32 value)	{ bottom += value - top ; top  = value; return value; }
		const INT32 set_w(INT32 value)	{ right  = left + value; return value; }
		const INT32 set_h(INT32 value)	{ bottom = top  + value; return value; }
		...
		void set_location(const Point& pt)	{ x = pt.x; y = pt.y; }
		void set_size(const Size& sz)		{ w = sz.w; h = sz.h; }
		void set_center(const Point& pt)	{ x = pt.x-w/2; y = pt.y-h/2; }
...
namespace mew
{
	...
	// 文字列への変換(Size).
	template <> class ToString<Size>
	{
	private:
		WCHAR m_str[80];
	public:
		ToString(const Size& value) throw()
		{
			swprintf(m_str, L"%d %d", value.w, value.h);
		}
		operator PCWSTR () const throw()	{ return m_str; }
	};

	// 文字列への変換(Point).
	template <> class ToString<Point>
	{
	private:
		WCHAR m_str[80];
	public:
		ToString(const Point& value) throw()
		{
			swprintf(m_str, L"%d %d", value.x, value.y);
		}
		operator PCWSTR () const throw()	{ return m_str; }
	};

	// 文字列への変換(Rect).
	template <> class ToString<Rect>
	{
	private:
		WCHAR m_str[160];
	public:
		ToString(const Rect& value) throw()
		{
			swprintf(m_str, L"%d %d %d %d", value.left, value.top, value.right, value.bottom);
		}
		operator PCWSTR () const throw()	{ return m_str; }
	};

	// 文字列への変換(Color).
	template <> class ToString<Color>
	{
	private:
