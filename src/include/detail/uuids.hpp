...
#define PP_EXPAND_NUMERIC(fn) \
  fn(INT8) fn(UINT8) fn(INT16) fn(UINT16) fn(INT32) fn(UINT32) fn(INT64) fn(UINT64) fn(REAL32) fn(REAL64)

#define PP_EXPAND_INTRINSIC(fn) fn(SIZE) fn(POINT) fn(RECT) fn(bool) PP_EXPAND_NUMERIC(fn)
...
template <>
struct std::less<GUID> : std::binary_function<GUID, GUID, bool> {
  bool operator()(REFGUID lhs, REFGUID rhs) const { return memcmp(&lhs, &rhs, sizeof(GUID)) < 0; }
};
